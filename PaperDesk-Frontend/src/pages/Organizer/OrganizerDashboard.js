...
const HeroBannerSkeleton = () => (
  <div className="relative overflow-hidden rounded-2xl min-h-[200px] flex items-center bg-muted/40 p-8 lg:p-12">
  ...
  <Card>
    ...
    <CardContent className="px-5 pb-5 space-y-4">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1.5 pt-1 border-t border-border">
        {[28, 24, 32].map((w, i) => (
          <Skeleton key={i} className="h-3.5 rounded-full" style={{ width: `${w * 3}px` }} />
        ))}
      </div>
    </CardContent>
  </Card>
...
const OrganizerDashboard = () => {
  ...
  // FIX: API returns `p.decision`, not `p.final_decision`
  const accepted    = papers.filter(p => p.decision === "Accepted").length;
  const rejected    = papers.filter(p => p.decision === "Rejected").length;
  const modRequired = papers.filter(p => p.decision === "Modification Required").length;

  // FIX: API returns p.status as "assigned" | "reviewed" | "pending" | "resubmitted"
  // "Under Review" = assigned to reviewer(s) but no final decision yet
  const underReview = papers.filter(p =>
    (p.status === "assigned" || p.status === "reviewed") &&
    (!p.decision || p.decision === "pending")
  ).length;

  // FIX: API returns p.reviewers[] array per paper, each with status "reviewed" | "pending"
  // There are no p.reviews_submitted or p.reviews_total fields
  const totalReviewsSubmitted = papers.reduce(
    (acc, p) => acc + (p.reviewers?.filter(r => r.status === "reviewed").length ?? 0), 0
  );
  const totalReviewsRequired = papers.reduce(
    (acc, p) => acc + (p.reviewers?.length ?? 0), 0
  );

  const decidedCount = accepted + rejected + modRequired;
  ...
  const donutSegments = [
    ...
    { color: "#94a3b8", pct: total > 0 ? (Math.max(0, total - decidedCount - underReview) / total) * 100 : 0, label: "Pending" },
  ];
                  ...
                  {!conferenceId ? (
                    ...
                    <>
                      ...
                      <ProgressBar
                        ...
                        sub={`${decidedCount} of ${total} papers have a final decision`}
                      />
